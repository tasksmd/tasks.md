...
- [ ] Ship a one-shot consumer migration command (file backend → git-native) (@devin)
  ...
  - **Files**: `commands/migrate.md` (new canonical), `commands/{claude,codex,cursor,devin,gemini,windsurf}/` (generated variants), `packages/cli/src/commands/generate-commands.ts`, `commands/README.md`, `commands/setup.md`, `README.md`, `AGENTS.md` (command propagation table)
  - **Acceptance**: `commands/migrate.md` exists as a canonical source and documents detect → dry-run → apply → `fleet init` → `doctor` → rollback; `npx tasks generate-commands` produces all per-agent variants with no drift (the `commands-drift` CI gate passes); `README.md`, `commands/README.md`, and `commands/setup.md` reference the command; `npm run build`, `npm test`, `npm run lint`, and `npx -y @tasks-md/lint TASKS.md` all pass.
...
- [ ] Dogfood: convert this repo's own queue to the git-native backend
  ...
  - **Blocked by**: consumer-migration-command
