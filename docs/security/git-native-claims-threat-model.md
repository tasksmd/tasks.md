...
| **`claim_id` is not a secret** — it is readable in the `tasks-claims` ref and surfaced by `tasks list --json` for owner convenience (retrieve your own token to heartbeat/fence) | By design: the token's job is *staleness detection* (a lapsed/replaced token stops matching), not secrecy. Forgery by a repo-read-capable actor is gated by the **server-side required check** (B5/B7) + signed identity (deferred), not by hiding the token — so client-side owner-gating of the read API would be theater (actor-id is advisory) | v1: documented non-goal. Real gate = Phase 3 server check |
